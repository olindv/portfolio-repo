<template lang="pug">
  .work
    ...
    form.work-form
      ...
      label.work-form__label.work-form__label_last
        input(type="file" @change="renderPicAndAddToData").work-form__input.work-form__input_file
        a.work-form__link
          img(src="../../assets/icons/upload.png").work-form__icon
          span.work-form__text Загрузить картинку
          span.preview(:style="{backgroundImage: preview.previewPic}")
      button(type="button" @click="editMode ? editCurrentWork(work) : addNewWork(work)").button {{editMode ? 'Сохранить изменения' : "Добавить"}}
</template>
